  ...
  def Hosts.configure(config, settings)
    ...
    settings['hosts'].each_with_index do |host, index|
      ...
      config.vm.define "#{host['name']}", autostart: autostart do |server|
        server.vm.box = host['box'] || 'peru/ubuntu-18.04-server-amd64'
