...
class Html
{
    ...
    private static function parseText($node, $element, &$styles)
    {
        ...
        if (is_callable(array($element, 'addText'))) {
            ...
            $element->addText(self::clean_text($node->nodeValue), $styles['font'], $styles['paragraph']);
        }
    }
        ...
        foreach ($properties as $property) {
            ...
            switch (trim($cKey)) {
                ...
                case 'font-family':
                    ...
                    break;
                case 'font' :
                    $fontattr = str_getcsv(trim($cValue, " \t\n\r\0\x0B;"), ' ');
                    $attrlist = array('font-family','font-size/line-height', 'font-weight', 'font-variant', 'font-style');
                    foreach($attrlist as $attr){
                        if(!empty($fontattr)){                            
                            $cVal = array_pop($fontattr);
                            switch ($attr){
                                case "font-family":
                                    $cVal = array_map('trim', explode(',', $cVal));
                                    $styles['name'] = ucwords($cVal[0]);
                                    break;
                                case "font-size/line-height":
                                    ...
                                    if(count($cValArr) > 1){
                                        ...
                                        if (preg_match('/([0-9]+\.?[0-9]*[a-z]+)/', $cValArr[1], $matches)) {
                                            ...
                                            if($cValArr[1] == "normal" || $cValArr[1] == "inherit" || $cValArr[1] == "initial"){
                                                $cValArr[1] = 1;
                                            }
                                            //any other, wich is a multiplier. E.g. 1.2
                                            $spacingLineRule = \PhpOffice\PhpWord\SimpleType\LineSpacingRule::AUTO;
                                            //we are subtracting 1 line height because the Spacing writer is adding one line
                                            $spacing = ($cValArr[1] * Paragraph::LINE_HEIGHT) - Paragraph::LINE_HEIGHT;
                                        }
                                        $styles['spacingLineRule'] = $spacingLineRule;
                                        $styles['line-spacing'] = $spacing;
                                    }else{
                                        $styles['size'] = Converter::cssToPoint($cValArr[0]);
                                    }

                                    break;
                                case "font-weight":
                                    $tValue = false;
                                    if (preg_match('#bold#', $cVal)) {
                                ...
                                case "font-style":
                                    $tValue = false;
                                    if (preg_match('#(?:italic|oblique)#', $cVal)) {
                                        $tValue = true;
                                    }
                ...
                case 'background-color':
                    ...
                    break;
                case 'background':
                    $bgattr = explode(' ', trim($cValue, " \t\n\r\0\x0B;"));
                    $styles['bgColor'] = trim($bgattr[0], '#');
                ...
                case 'line-height':
                    ...
                    if (preg_match('/([0-9]+\.?[0-9]*[a-z]+)/', $cValue, $matches)) {
                        ...
                        $spacingLineRule = \PhpOffice\PhpWord\SimpleType\LineSpacingRule::EXACT;
                        $spacing = Converter::cssToTwip($matches[1]);
                    } elseif (preg_match('/([0-9]+\.?[0-9]*?)%/', $cValue, $matches)) {
                        //matches percentages
                        $spacingLineRule = \PhpOffice\PhpWord\SimpleType\LineSpacingRule::AUTO;
                        //we are subtracting 1 line height because the Spacing writer is adding one line
                        $spacing = ((((int) $matches[1]) / 100) * Paragraph::LINE_HEIGHT) - Paragraph::LINE_HEIGHT;
                    } else {
                        if($cValue == "normal" || $cValue == "inherit" || $cValue == "initial"){
                            $cValue = 1;
                        }
                        //any other, wich is a multiplier. E.g. 1.2
                        $spacingLineRule = \PhpOffice\PhpWord\SimpleType\LineSpacingRule::AUTO;
    ...
    private static function parseLink($node, $element, &$styles)
    {
        ...
        if (strpos($target, '#') === 0) {
            return $element->addLink(substr($target, 1), self::clean_text($node->textContent), $styles['font'], $styles['paragraph'], true);
        }

        return $element->addLink($target, self::clean_text($node->textContent), $styles['font'], $styles['paragraph']);
    }
