...
class adLDAP {
    ...
    */
    ...
    /**
     * Optional ldap directory specific port
     * By default it is 389 and 636 for ssl
     * 
     * @var integer
     */
    protected $_domain_port = 389;
    ...
    */
    function __construct($options=array()){
        ...
        if (count($options)>0){
            ...
            if (array_key_exists("domain_port",$options)){ $this->_domain_port=$options["domain_port"]; }
            ...
            if ($this->_use_ssl === false && $this->_domain_port == 636){ $this->_use_ssl=true;} //override use_ssl if we see a secure port specified			
        }
    ...
    */
    public function connect() {
        ...
        if ($this->_use_ssl){
            ...
            $this->_conn = ldap_connect($dc, $this->_domain_port);
        }
        ...
        if ($this->_ad_username!=NULL && $this->_ad_password!=NULL){
            ...
            if (!$this->_bind){
            	//throw new \Exception(ldap_errno($this->_conn));
            	//throw new \Exception(ldap_error($this->_conn)); 
...
//                throw new adLDAPException('Rebind to Active Directory failed. AD said: ' . $this->get_last_error());
    ...
    */
    public function group_members($group, $recursive = NULL){
        ...
        for ($i=0; $i<$users["count"]; $i++){
        	 $name_parts = preg_split('~\\\\.(*SKIP)(*FAIL)|,~s',$users[$i]);
        	 foreach ($name_parts as $part) {
        	 	if (false !== stripos($part, 'CN=')) {
        	 		$cn = str_replace('CN=', '', $part);
        	 	}
        	 }
             $filter="(&(objectclass=person)(|(distinguishedName=".$this->ldap_slashes($users[$i]).")(cn=".$this->ldap_slashes($cn).")))";
             $fields = array("samaccountname", "distinguishedname", "objectClass","givenname","sn", "uid","displayname","mail","cn");
             $sr=ldap_search($this->_conn,$this->_base_dn,$filter,$fields);
             $entries = ldap_get_entries($this->_conn, $sr);

             // not a person, look for a group  
             if ($entries['count'] == 0 && $recursive == true) {  
                $filter="(&(|(objectCategory=group)(objectClass=dominoGroup))(|(distinguishedName=".$this->ldap_slashes($users[$i]).")(displayname=".$this->ldap_slashes($cn).")))";  
                $fields = array("samaccountname", "cn");  
                $sr=ldap_search($this->_conn,$this->_base_dn,$filter,$fields);  
                $entries = ldap_get_entries($this->_conn, $sr);
                if (!isset($entries[0]['samaccountname'][0]) && !isset($entries[0]['cn'][0])) {
                    continue;  
                }
                $sub_users = $this->group_members($entries[0]['cn'][0], $recursive);  
             ...
             elseif ($entries[0]['samaccountname'][0] === NULL && $entries[0]['cn'][0] !== NULL) {
                $dn = preg_split('~\\\\.(*SKIP)(*FAIL)|,~s', $entries[0]['dn']);
             	$o = '';
             	foreach ($dn as $part) {
             		if (false !== stripos($part, 'O=')) {
             			$o .= '/'.str_replace('O=', '', $part);
             		}
             	}
             	$user_array[] = $entries[0]['cn'][0] . $o;
             }
    ...
    * @return array
    */
    public function group_info($group_name = null, $fields=NULL, $filter = null)
    {
        if (is_null($group_name) && is_null($filter)){ return (false); }
        if (!$this->_bind){ return (false); }
        ...
        if (!empty($group_name)) {
        	...
        	$filter = "(&(|(objectCategory=group)(objectClass=dominoGroup))(|(samAccountName=".$this->ldap_slashes($group_name).")(cn=".$this->ldap_slashes($group_name).")(name=".$this->ldap_slashes($group_name).")(displayname=".$this->ldap_slashes($group_name).")))";
        }
    ...
    * @return array
    */
    public function search_groups($samaccounttype = ADLDAP_SECURITY_GLOBAL_GROUP, $include_desc = false, $search = "*", $sorted = true, $filter = null) {
        if (!$this->_bind){ return (false); }
        ...
        ldap_control_paged_result($this->_conn, 10000);
        // Perform the search and grab all their details
        $fields=array("samaccountname","description","displayname","cn", "name");
        $sr=ldap_search($this->_conn,$this->_base_dn,$filter,$fields,0,0);
        $entries = ldap_get_entries($this->_conn, $sr);

        $groups_array = array();        
        for ($i=0; $i<$entries["count"]; $i++){
        	$gname = (!empty($entries[$i]["displayname"]) ? $entries[$i]["displayname"][0] : (!empty($entries[$i]["name"]) ? $entries[$i]["name"][0] : (!empty($entries[$i]["cn"]) ? $entries[$i]["cn"][0] : $entries[$i]["samaccountname"][0])));
        	$groups_array[$gname] = ($include_desc && strlen($entries[$i]["description"][0]) > 0 ?  $entries[$i]["description"][0] : $entries[$i]["cn"][0]);
        }
...
//            throw new adLDAPException('SSL must be configured on your webserver and enabled in the class to set passwords.');
    ...
    */
    ...
    public function getUserDominoGroups($username){
    	
     if ($username===NULL){ return (false); }
     if (!$this->_bind){ return (false); }
     
     $filter = "member=".$userDetails["DN"];
     
     $sr=ldap_search($this->_conn," ",$filter,array("dominoAccessGroups"));
     $entries = ldap_get_entries($this->_conn, $sr);
    	
    	
    }
    
    public function getUserInfoDomino ($username){
   		if ($username===NULL){ return (false); }
     	if (!$this->_bind){ return (false); }
     
     	$filter = "cn=".$username;
     	$sr=ldap_search($this->_conn," ",$filter);
     	$entries = ldap_get_entries($this->_conn, $sr);
     	  if (isset($entries[0])) {
...
//            throw new adLDAPException('SSL must be configured on your webserver and enabled in the class to set passwords.');
    ...
    */
     ...
     */
    public function search_user($filter, array $attributes = array(), $limit = 0)
    {
    	if (empty($filter))
    	{
    		return false;
        ...
        if (empty($attributes)) 
        {
        	//$attributes = array("uid","cn","distinguishedname","mail","memberof","department","displayname","telephonenumber","primarygroupid","objectsid","sAMAccountName","givenname","sn"); 
        	$attributes = array( "samaccountname","givenname","sn", "uid","displayname","mail","cn","userprincipalname");
       	}
       	
        $search = ldap_search($this->_conn, $this->_base_dn, $filter, $attributes);
        ldap_sort($this->_conn, $search, 'cn');
        if ($search) {
    ...
    */
    public function find_base_dn() {
        ...
        $_dn = ((empty($namingContext) || empty($namingContext[0]) || empty($namingContext[0]['defaultnamingcontext']) || empty($namingContext[0]['defaultnamingcontext'][0])) ? '' : $namingContext[0]['defaultnamingcontext'][0]); 
        return $_dn;
    }
    ...
    */
    protected function random_controller(){
    	if (!empty($this->_domain_controllers)) {
