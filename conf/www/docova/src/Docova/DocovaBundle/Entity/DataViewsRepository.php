 ...
 */
class DataViewsRepository extends EntityRepository
{
	public function getDataView($folder_id){
		$qbLib = $this->createQueryBuilder("d");
		$qbLib->select("d")
		->where("d.Folder_Id=?1")
		->setParameter(1, $folder_id);
	...
	{
		return $this->getEntityManager()->getRepository("DocovaBundle:DataSources")->getDataSources();
	}
			...
			if (!empty($sortBy))
			{
				foreach ($sortBy as $key => $value) {
					if ($this->_class->hasField($key)) {
						...
						elseif ($key == 'Data_Source_Name' && !empty($value)) {
							$orderBy['Data_Source_Name'] = strtoupper($value) == 'DESC' ? 'DESC' : 'ASC';
						}
					}
				}
			}
