...
function UploaderOnBeforeAdd(filePath, targetUploader){
	...
	if(!docInfo.isFileCIAOEnabled) {return IsValidFileCount(Uploader);} // nothing to process
	...
	if(!fileExists && !isCheckedOut) {return IsValidFileCount(Uploader);}
...
function IsValidFileExtension(fileName, extensionListOrTargetUploader){	
	...
	var extensionArray= allowedext.split(",");
...
function HandleScanClick(targetUploader){
	EZTwainScanImages(targetUploader);
	
}

function EZTwainScanImages(targetUploader){
	var Uploader = (targetUploader && targetUploader.FileCtrlId ? targetUploader : (typeof DLIUploader != "undefined" ? DLIUploader1 : null));
...
function GetTemplateInfo()
{
	...
	if(docInfo.TemplateList.split(";").length==1)
	...
	else
		{
			...
			var selValue = thingFactory.SelectKeyword(docInfo.TemplateNameList, ";", "Select template", "", false, false, false );
...
function GetTemplateUrl(templateInfo)
{
	...
	url += "/luFileTemplatesByKey/" + templateInfo[0] + "/$File/" + templateInfo[1];
	return url;
}
...
function UploaderRedirectDownload(fileName, targetUploader){
	...
	if(docInfo.Mode=="preview"){
		Uploader.LaunchUrl=docInfo.DataDocUrl + "/$file/" + escape(fileName) + "?OpenElement&Login";
	}
	return(true);
}
...
function SingleFileUploader(containerdiv)
{
	var contdiv = "";
	...
	this.init =  function (containerdiv){
		this.contdiv = containerdiv;
		this.editcontainer = this.contdiv.find(".singleuploaderedit");
		this.displaycontainer = this.contdiv.find(".singleuploaderdisplay");
		this.editcontainer.hide();
		this.displaycontainer.show();

		var index = containerdiv.attr("upcontrolno");
		this.upobj =  window["DLIUploader" + index];
		this.upobj.forceNative = true;
		this.upobj.pluginEnabled = false;
		this.allfiles = this.upobj.GetAllFileNames();
		if ( this.allfiles.length > 0 )
		{
			var ifrm = this.displaycontainer.find("iframe");
			ifrm.attr("src", "/" + docInfo.NsfName + "/openDocFile/" + this.allfiles + "?OpenElement&doc_id=" + $$DocumentUniqueID());
			this.editcontainer.find("#singleupcurrfile").text("Current File : " + this.allfiles );
			this.editcontainer.hide();
		}else{

			this.editcontainer.find("#singleupcurrfile").text("Current File : None");
			this.editcontainer.show();
			this.displaycontainer.hide();
			if ( ! docInfo.isDocBeingEdited )
			{
				this.editcontainer.html ( "<p style='text-align:center;font-size:16px; color:darkgray'>No File Attached!</p>");
			}

		}

		this.edtbtn = this.displaycontainer.find("#btn_singleupedit");
		if ( ! docInfo.isDocBeingEdited )
		{
			this.edtbtn.hide();
		}

		this.dlgbtn = this.displaycontainer.find("#btn_singleupdialog");
		this.btnadd = this.contdiv.find(".btnsingleattachmentadd")
	...
	this.hookevents = function ()
	{
		...
		var that = this;
		this.edtbtn.click ( function () {
			that.editcontainer.show();
			that.displaycontainer.hide();	
		});	

		
		this.dlgbtn.click ( function () 
		{

			if ( that.allfiles.length == 0 ) return;
			var dlghtml = "<div id='invdisplay' style='width:100%;height:140px;margin-top:8px;background:white;overflow:hidden;'>";
			dlghtml += "<iframe id='invoicerender2' style='width:100%;height:100%;' src='/"+ docInfo.NsfName +"/openDocFile/" + that.allfiles + "?OpenElement&doc_id=" + $$DocumentUniqueID() + "'" + "></iframe>";

			$("body", window.top.document).append(dlghtml);


		    $("#invdisplay", window.top.document).dialog({ 
		         autoOpen: true, 
		         width : 900,
		         height: 300,
		         resizable : true,
		...
		this.btnadd.click ( function (event) {
			...
			that.upobj.AddFile();
			//$("#" + that.upobj.FileCtrlId).click();
		});

		this.upobj.onAdd = function (filename, flobj) 
		{

			var ifrm = that.displaycontainer.find("iframe");
			var reader = new FileReader();
        	var mime= flobj.type;
        	var imgobj = that.displaycontainer.find("#suimagepreview");
	        ...
	        reader.onload = function (e) 
	        {
	            ...
	            {
	               ifrm.attr('src',  e.target.result);
	               imgobj.hide();
	               ifrm.show();

	            }
	            that.displaycontainer.show();
				that.editcontainer.hide();
	        }
        
        	reader.readAsDataURL(flobj);

			
			that.contdiv.find("#singleupcurrfile").html ( "Current File : " + filename);
		}

		this.upobj.onBeforeAdd = function ( filename){
			that.upobj.SelectAll();
			that.upobj.RemoveSelectedFiles();
			return true;
		}
  ...
  for(var i=0; i<uploaderconfigs.length; i++){
  		if(typeof uploaderconfigs == "object"){
			...
			if($targetdiv.length > 0){
				...
				if ( ! $targetdiv.attr("onefilemode") ||  $targetdiv.attr("onefilemode") !== "true"){
					$targetdiv.show();
				}else{
					//single file uploader mode
					
					var singleup = $targetdiv.parent();
					
					var suploder = new SingleFileUploader(singleup);
